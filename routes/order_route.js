...
router.post('/list', async (req, res) => {
    try {
       const result= await Order.adminOrder(db);
...
router.post('/cartidlist/:id', async (req, res) => {
    try {
        console.log(req.params.id);
       const result= await Order.CartIdList(db,req.params.id);
...
router.post('/updatecart', async (req, res) => {
    try {
       const result= await Order.addToCart(db,req.body);
        
        if (!result) {
            res.status(403).json({ "message": "No cartId" });
        } else {
...
router.post('/deletecart/:id', async (req, res) => {
    try {
       const result= await Order.deleteCart(db,parseInt(req.params.id));
...
router.post('/updatestat', async (req, res) => {
    try {
       const result= await Order.updateStat(db,req.body);
        
        if (!result) {
            res.status(403).json({ "message": "No irderId" });
        } else {
